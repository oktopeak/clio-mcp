...
describe("create_folder if_not_exists", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockClioPost.mockResolvedValue({ data: { id: 900, name: "Pleadings", parent: { id: 42, type: "Matter" } } });
  });
  ...
  it("creates the folder when the name is free", async () => {
    ...
    const parsed = JSON.parse(result.content[0].text);
    expect(mockClioPost).toHaveBeenCalledTimes(1);
    expect(parsed.created).toBe(true);
    expect(parsed.folder.id).toBe(900);
  });

  // Leon Mead's report: a dry run reported nearly every matter missing a folder
  // that was present, and a write pass against that would have created duplicate
  // folders across several hundred matters. Enforcing the check inside the tool
  // is what makes a repeated bulk run safe; telling the caller to check first is
  // not, because over hundreds of matters the caller eventually does not.
  it("returns the existing folder instead of creating a second one", async () => {
    mockClioGetAllPages.mockResolvedValue([{ id: 500, name: "Pleadings", parent: { id: 77, type: "Folder" } }]);
    const result = await handlers["create_folder"]({ name: "Pleadings", matter_id: 42, if_not_exists: true }) as any;
    const parsed = JSON.parse(result.content[0].text);
    expect(mockClioPost).not.toHaveBeenCalled();
    expect(parsed.created).toBe(false);
    expect(parsed.folder.id).toBe(500);
  });

  it("finds an existing folder whose parent is a Folder, not the Matter", async () => {
    // A matter's document root can itself be a Folder node, so its children
    // report parent.type "Folder". Filtering on parent type here is exactly what
    // produced false negatives at scale, so the lookup must not do it.
    mockClioGetAllPages.mockResolvedValue([{ id: 501, name: "Discovery", parent: { id: 77, type: "Folder" } }]);
    const result = await handlers["create_folder"]({ name: "Discovery", matter_id: 42, if_not_exists: true }) as any;
  ...
  it("skips the lookup entirely when if_not_exists is off", async () => {
    await handlers["create_folder"]({ name: "Pleadings", matter_id: 42, if_not_exists: false });
    expect(mockClioGetAllPages).not.toHaveBeenCalled();
    expect(mockClioPost).toHaveBeenCalledTimes(1);
  });

  it("paginates the existence check to completion", async () => {
    mockClioGetAllPages.mockResolvedValue([]);
    await handlers["create_folder"]({ name: "Pleadings", matter_id: 42, if_not_exists: true });
    // clioGetAllPages, not clioGet: a first-page-only read under-counts, which
    // is a false negative on an existence check and a duplicate on the write.
