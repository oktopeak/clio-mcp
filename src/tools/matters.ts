  ...
  server.registerTool(
    "create_matter",
    {
      ...
      inputSchema: {
        ...
        matter_stage_id: z.number().int().positive().optional().describe("Clio matter stage ID (see list_matter_stages). Confirmed live: setting this through the API fires the same Clio automations (e.g. \"assign a task list\") attached to that stage in Clio\u2019s Settings, exactly as a UI-driven stage change would. An automation can still fail for unrelated reasons \u2014 e.g. it references \"Originating Solicitor\" but the matter has no originating_attorney set \u2014 the same way it would if triggered from the UI"),
